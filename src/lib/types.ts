...
export type AssertionFunction<O extends Role = Role, R extends Resource = Resource> = (hrbac : HRBAC, role : O|null, resource : R|null, privilege : string|null) => boolean;

export class Assertion<O extends Role = Role, R extends Resource = Resource> {
  constructor(public assert : AssertionFunction<O, R>) {}
}

export type AsyncAssertionFunction<O extends Role = Role, R extends Resource = Resource> = (hrbac : AsyncHRBAC, role : O|null, resource : R|null, privilege : string|null) => Promise<boolean>|boolean;
